...
def highlight_matches(file1_path, file2_path, col1_name, col2_name):
    try:
        print(f"⚙️ Confronto file: {file1_path} vs {file2_path}")
        df1 = pd.read_excel(file1_path)
        df2 = pd.read_excel(file2_path)

        print(f"📊 Colonne disponibili in File 1: {df1.columns.tolist()}")
        print(f"📊 Colonne disponibili in File 2: {df2.columns.tolist()}")

        if col1_name not in df1.columns or col2_name not in df2.columns:
            return None, f"Errore: colonna '{col1_name}' o '{col2_name}' non trovata."
        ...
        wb.save(output_path)
        wb.close()
        print("✅ Confronto completato con successo!")

        return output_path, None

    except Exception as e:
        print(f"❌ Errore durante il confronto: {str(e)}")
...
def upload_files():
    try:
        ...
        print(f"📂 File salvati: {file1_path}, {file2_path}")
